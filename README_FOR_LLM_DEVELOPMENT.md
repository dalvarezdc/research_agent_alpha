...
1. **PDF Generation** ⭐ **NEW**
   - All markdown output files MUST have a corresponding PDF version
   - PDF generation happens automatically via `pdf_generator.py`
   - PDFs use custom CSS with emoji support and medical report styling
   - Both .md and .pdf files are saved to the same output directory

   **Automatic Implementation:**
   ```python
   from pdf_generator import convert_markdown_to_pdf_safe

   # After saving markdown file
   summary_file = f"{output_dir}/report_{timestamp}.md"
   ...
   if pdf_file:
       print(f"✓ PDF generated: {os.path.basename(pdf_file)}")
   ```

   **Dependencies Required:**
   - `markdown>=3.5.0` - Convert markdown to HTML
...
2. **Emoji Formatting for Titles and Subtitles** ⭐ **REQUIRED**
   ...
   **Implementation in Report Generators:**
   ```python
   ...
   **Implementation in LLM Prompts (for agents that generate prose):**
   ```python
   def _generate_output(self, subject: str, data: Dict) -> str:
       """Generate prose output with emojis"""
       prompt = f"""
       Write a comprehensive guide for {subject}.
   ...
   **Key Principle:** For agents that generate markdown/prose directly (like fact checker),
   include emoji instructions in BOTH the prompt and system prompt. For agents that generate
   structured data (like medication analyzer), emojis are added in the report wrapper functions.

2. **References Collection Per Phase** ⭐ **REQUIRED**
   - References MUST be collected in EVERY analysis phase
   - Each phase should request 3-5 specific citations from the LLM
   - References are extracted and aggregated automatically
   - Final output includes comprehensive, deduplicated reference list

   **Problem This Solves:**
   - ❌ OLD: References added as afterthought, often incomplete or generic
   - ✅ NEW: References collected systematically during each phase of analysis
   ...
   **Implementation Pattern:**
   ...
   class PhaseResult:
       ...
       references: List[Dict[str, Any]] = field(default_factory=list)  # ← CRITICAL

   def _phase_analysis(self, subject: str) -> PhaseResult:
       """Execute analysis phase with reference collection"""
       prompt = f"""
       Analyze {subject}...
       ...
       return PhaseResult(
           phase=AnalysisPhase.EVIDENCE_GATHERING,
           timestamp=datetime.now(),
           content=parsed_content,
           references=references  # ← Store extracted references
       )

   def _extract_references_from_text(self, text: str) -> List[Dict[str, Any]]:
       """Extract references from text response using regex parsing"""
       import re
       references = []
       # Look for REFERENCES: section
       # Extract DOI (10.xxxx/xxx), PMID (PMID: xxxxx), URLs
       # Return structured reference dicts with raw_citation, doi, pmid, url, year
       return references
   ```
   ...
   **Aggregation in Reports:**
   ...
   def _append_references_section(self, output: str, session: Any) -> str:
       ...
       for phase_result in session.phase_results:
           if hasattr(phase_result, 'references'):
               for ref in phase_result.references:
                   # Deduplicate by DOI/PMID/title
                   unique_key = ref.get('doi') or ref.get('pmid') or ref['raw_citation'][:100]
                   if unique_key not in seen_refs:
                       seen_refs.add(unique_key)
                       all_references.append(ref)

       # Format and append to output
       return output + formatted_references
   ```

   **Benefits:**
   - ✅ Complete reference lists (not afterthoughts)
   - ✅ Specific, verifiable citations with DOI/PMID
   - ✅ Automatic deduplication across phases
   - ✅ Traceability of evidence sources
   - ✅ Better integration with reference validation

3. **References Section Format** (at the end of report)
   ```markdown
   ## 📚 References
   [1] Smith J, et al. (2020). Study Title. Journal Name. https://doi.org/10.1234/example
   [2] Jones A, et al. (2021). Another Study. PMID: 12345678
   [3] CDC Guidelines. https://www.cdc.gov/resource
   ```

4. **Cost Analysis Section** (in report metadata or footer)
   ```markdown
...
- Added Reference and ReferencesCollection Pydantic schemas to dspy_schemas.py
- Updated medical_fact_checker_agent.py to collect references per phase
- Added _extract_references_from_text() helper for regex-based reference extraction
- Updated PhaseResult dataclass to include references field
- Updated run_analysis.py to aggregate and deduplicate references from all phases
- All phase prompts now request 3-5 specific citations with DOI/PMID/URLs
- References are now complete, verifiable, and traceable to specific phases
...
- Created pdf_generator.py with custom medical report CSS styling
- Updated pyproject.toml with markdown, weasyprint, and pygments dependencies
- All agents now generate both .md and .pdf versions of reports
